...
public class Board_3 {
    ...
    public Board_3(QuadsManagement quadManagement, Maths maths, ArrayList<Player> playerList, ArrayList<Sun> sunList) {
        this.maths = maths;
    ...
    public void setupSunChildren() {
        ...
        for(Sun sun : sunList)
        {
            float randF = random.nextFloat() / 3;
        ...
        {
            if(random.nextInt(2) == 0) {
                bVec.x += random.nextFloat() / 4;
            }else {
                bVec.x -= random.nextFloat() / 4;
            }
            if(random.nextInt(2) == 0) {
                bVec.y += random.nextFloat()/4;
            } else {
                bVec.y -= random.nextFloat()/4;
            }
        }
    ...
    public void buildBorder(Player player)
    {
      ...
      player.bordersList.clear();
            ...
            {
                Sun cSun = player.cStarList.get(k);
                addColonyBorder(player, cSun);
            }
        ...
        for (int i = 0; i < player.bordersList.size(); i++)
        {
            ...
            for (int j = 0; j < player.bordersList.size(); j++)
            {
                ...
                if (Math.abs(bord_1.curentPosition.x - bord_2.curentPosition.x) < 0.001f && Math.abs(bord_1.curentPosition.y - bord_2.curentPosition.y) < 0.001f) {
                    ...
                    //if(bord_1.volatileBorder  && bord_2.volatileBorder || !bord_1.volatileBorder  && !bord_2.volatileBorder)
                    //{
    ...
    public void getStarNeighbors(Sun centerStar) {
        boolean done = false;
        while (!done) {
            closestAngleS = null;
            minAngle = 360;
            for (Sun sun : sunList) {
                if (centerStar.position.x == sun.position.x && centerStar.position.y == sun.position.y) {
                    continue;
                }
                distance = maths.calcPointsPointDistance(centerStar.position, sun.position);
                if (distance < scanDistance && distance > 0) {
                    curentAngle = maths.calcPointPointAngle(sun.position, centerStar.position);
                    if (curentAngle < minAngle) {
                        if (curentAngle > centerStar.borderAngle) {
                            minAngle = curentAngle;
                            closestAngleS = sun;
                        }
                    }
                }
            }
            if (closestAngleS == null) {
                for (Sun sun : sunList) {
                    if (centerStar.position.x == sun.position.x && centerStar.position.y == sun.position.y) {
                        continue;
                    }
                    distance = maths.calcPointsPointDistance(sun.position, centerStar.position);
                    if (distance < scanDistance && distance > 0) {
                        curentAngle = maths.calcPointPointAngle(sun.position, centerStar.position);
                        if (curentAngle < minAngle) {
                            minAngle = curentAngle;
            ...
            if (closestAngleS != null) {
                if (centerStar.sunNeighborsList.size() == 0) {
                    centerStar.borderAngle = minAngle;
                    centerStar.sunNeighborsList.add(closestAngleS.position);
                } else {
                    if (closestAngleS.position.x == centerStar.sunNeighborsList.get(0).x && closestAngleS.position.y == centerStar.sunNeighborsList.get(0).y) {
                        done = true;
                    } else {
                        centerStar.borderAngle = minAngle;
                        centerStar.sunNeighborsList.add(closestAngleS.position);
