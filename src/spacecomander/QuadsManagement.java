...
public class QuadsManagement {
    ...
    Quad ship,ring,bullet,explosion,interceptor,shield,phaser,border,emiter,shipSelectRing,lineSegment,point,quad;
    Model model;
    IBO      quad_ibo , triangle_ibo;
    ArrayList<Quad>      colonizedInfluenceRing;
    ArrayList<Quad>      shipSelectionRing;

    TextureUtil textureUtil;
    ArrayList<Player>    playerList;
    ArrayList<Quad>    linePrimitive , pointPrimitive;

    float scale;
    
    public float scaleQuad;
    int race_index;
    
    IBO_Data IBO_Data  ;
    IBO      ibo;
    
    public QuadsManagement( ArrayList<Quad>    colonizedInfluenceRing  ,  IBO  quad_ibo , IBO  triangle_ibo , TextureUtil textureUtil , float scaleGame , ArrayList<Player>    playerList  , ArrayList<Quad>      shipSelectionRing ,  int race_index , ArrayList<Quad>  linePrimitive ,ArrayList<Quad> pointPrimitive){
       this.colonizedInfluenceRing = colonizedInfluenceRing;
       this.shipSelectionRing = shipSelectionRing;
       this.linePrimitive = linePrimitive;
       this.pointPrimitive = pointPrimitive;
    ...
    public void addShipSelectRing(Quad ship)
    {
        ...
        model = new Model(quad_ibo, textureUtil.ringTexture , playerList.get(race_index).selectyRing, shipSelectRing.curentPosition, ship.quadRotation, ship.scale);

        //System.out.println(playerList.get(race_index).ring_index);
        shipSelectRing.model = model;
        shipSelectRing.quadPlayerIndex = playerList.get(ship.quadPlayerIndex).index;

        ship.selectRing = shipSelectRing;

        shipSelectionRing.add(shipSelectRing);
    ...
    public void addSunRing(Planet planet , Player player , Sun sun , float   size) {
        ...
        if(sun.ring == null)
        {
            ...
            colonizedInfluenceRing.add(ring);
        } else {
    ...
    public void addPlanetRind(Player player , Planet planet)  {
        ...
        if(planet.player != null  && planet.owner != player.index)
        {
           ...
           colonizedInfluenceRing.remove(planet.ring);
           planet.ring = null;
        }
    ...
    public void addPoint(float   x , float   y , float w , int textIndex) {  
        point = new Quad();
        point.scale = new Vector3f(w,w,1);
        model = new Model(quad_ibo, textureUtil.board , textIndex, point.curentPosition, point.quadRotation, point.scale);
        point.model = model;
        pointPrimitive.add(point); 
    }
    ...
    public void addPhaser(Quad ship , ArrayList<Quad>  phaserList)
    {
        phaser = new Quad();
   ...
   public Quad  addColonizationShip(Planet planet ,  Player player  ,  float x , float y)   {
    ...
    public void addExploreShip(Planet planet ,  Player player  ,  float x , float y) 
    {
        ship = new Quad();
        ship.curentPosition.x = x;
        ship.curentPosition.y = y;
    ...
    public void addFreagateShip(Planet planet ,  Player player  ,  float x , float y) {
        ship = new Quad();
        ship.curentPosition.x = x;
        ship.curentPosition.y = y;
        ship.curentPosition.z = 0.003f;
        ...
        ship.name = "fregate ship";
        player.shipList.add(ship);  
    
    }
    public void addDestroierShip(Planet planet ,  Player player  ,  float x , float y) {
        ship = new Quad();
        ship.curentPosition.x = x;
        ship.curentPosition.y = y;
        ship.curentPosition.z = 0.003f;
        ...
        ship.deployPlanet = planet;
        ship.faction = player.faction;
        ship.name = "destroier ship";
        ship.bullet_attack_reset = 300;
        player.shipList.add(ship);

    }
    public void addCarrierShip(Planet planet ,  Player player  ,  float x , float y) {
        ship = new Quad();
        ship.curentPosition.x = x;
        ship.curentPosition.y = y;
        ship.curentPosition.z = 0.003f;
    ...
    public void addMotherShip(Planet planet ,  Player player  ,  float x , float y) {
         ship = new Quad();
        ship.curentPosition.x = x;
        ship.curentPosition.y = y;
        ship.curentPosition.z = 0.003f;
